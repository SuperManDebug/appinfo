$(function(){
	...
	$.ajax({
		type:"GET",//请求类型
		url:ctx+"/AppCategory/datadictionarylist",//请求的url
	...
	$.ajax({
		type:"GET",//请求类型
		url:ctx+"/AppCategory/categorylevellist",//请求的url
		data:{pid:null},//请求参数
		dataType:"json",//ajax接口（请求url）返回的数据类型
		success:function(data){//data：返回数据（json对象）
			$("#categoryLevel1").html("");
	...
	$("#categoryLevel1").change(function(){
		...
		if(categoryLevel1 != '' && categoryLevel1 != null){
			$.ajax({
				type:"GET",//请求类型
				url:ctx+"/AppCategory/categorylevellist.json",//请求的url
	...
	$("#back").on("click",function(){
        window.location.href = ctx+"/AppCategory/appLevenOneList?opt=dev";
	});
	...
	$("#APKName").bind("blur",function(){
		...
		$.ajax({
			type:"GET",//请求类型
			url:ctx+"/AppCategory/apkexist",//请求的url
